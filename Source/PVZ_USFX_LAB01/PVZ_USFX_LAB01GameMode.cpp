...
void APVZ_USFX_LAB01GameMode::BeginPlay()
{
	...
	for (int i = 0; i < 5; i++) {
		...
		for (int j = 0; j < 2; j++) {
			...
			UE_LOG(LogTemp, Warning, TEXT("Energía de %i: %i"), *NombrePlanta, NuevoPlant->energia);

				// Muestra un mensaje en la pantalla 
			GEngine->AddOnScreenDebugMessage(-1, 15.4, FColor::Yellow, FString::Printf(TEXT("Energia de %s: %1"), *NombrePlanta, NuevoPlant->energia));

				// Agrega la planta al contenedor de plantas

			// Plantas.Add(NombrePlanta, NuevoPlant);
